        ...
        <section className="unlock-panel">
          ...
          {!isConnected ? <div className="unlock-empty"><LockKey size={28} /><h3>Connect your wallet to start.</h3><p>The test uses free demo shares on Base Sepolia. Connect using the button above.</p></div> : <>
            ...
            <div className="unlock-steps">
              <article className={amount >= required ? "done" : ""}><span>{amount >= required ? <Check size={15} /> : "1"}</span><div><h3>Get a free test share</h3><p>Skip this if your wallet already has demo AAPLc. Need test ETH for the transaction? <a href="https://faucet.zalalena.com/base" target="_blank" rel="noreferrer">Open the Base Sepolia faucet</a>.</p></div><button type="button" onClick={() => void getShare()} disabled={transactionBusy || amount >= required}>{amount >= required ? "Received" : "Get share"}</button></article>
